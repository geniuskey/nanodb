...
- 측정은 마우스 클릭 전용이며 키보드만으로는 시작할 수 없습니다(좌표 직접 입력 대안은 UIX-003으로 계획). 태블릿·모바일 터치 측정과 다크모드는 지원 범위가 아닙니다.
- 등록 폼의 오류는 폼 하단에 한 줄로 표시되며 어떤 입력이 문제인지 필드 옆에 표시하지 않습니다(IMG-009·IMG-010으로 계획).
