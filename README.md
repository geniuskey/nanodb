...
6. 화살표·원 도형으로 관심 위치를 표시하고 도형마다 제품·Step·측정 항목 명을 남깁니다.
7. 홈에서 실제 이미지 수, 측정 수와 항목별 평균·최소·최대를 확인합니다.
8. 선택 이미지의 명세·측정 데이터·개발 요청·검증 기준을 담은 ZIP을 내보냅니다.
...
단일 키워드 검색·종류 필터, 측정 삭제, 이미지 삭제, 항목별 표본 수·평균·최소·최대와 도형 라벨링은 P1으로 구현을 마쳤습니다. 홈 최상단 소개 영상은 외부 임베드이므로 네트워크가 없으면 표시되지 않지만, 홈의 나머지 내용과 앱 기능은 그대로 동작합니다.
...
- 인증·권한은 이번 범위 밖입니다.
- 측정 화면에 확대·이동이 없어 원본이 화면보다 크면 화면 1px이 원본 여러 px에 대응합니다. 원본 1px 단위 지정은 아직 어렵습니다(MEA-013·MEA-014로 계획).
- 저장된 측정은 메모 외에는 수정할 수 없습니다. 잘못 찍은 측정은 삭제 후 다시 측정합니다.
- 그린 도형을 삭제하는 기능과 도형을 개발 컨텍스트 ZIP에 포함하는 기능은 아직 없습니다(ANN-005·ANN-008로 계획).
- 측정은 마우스 클릭 전용이며 키보드만으로는 시작할 수 없습니다. 태블릿·모바일 터치 측정과 다크모드는 지원 범위가 아닙니다.
