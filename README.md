...
홈 최상단 소개 영상은 저장소에 포함된 로컬 파일(`assets/video/nanodb_intro_h264.mp4`, 원본 녹화본 `nanodb_intro.mp4`의 H.264 사본)을 재생합니다. 원본은 HEVC라 Safari 밖에서는 재생하지 못하는 브라우저가 많아 어디서나 재생되는 사본을 번들에 넣습니다(자세한 사정과 재생성 명령은 `assets/video/README.md`). 외부 임베드나 서드파티 스크립트를 쓰지 않으므로 **네트워크가 차단된 환경에서도 그대로 재생됩니다.** 영상은 보조 자료이므로 재생하지 않고 홈 내용만으로 진행해도 되며, 그 안내가 영상 아래에 항상 표시됩니다. 시스템에서 동작 줄이기(reduced motion)를 켠 환경에서는 자동 재생하지 않고 재생 버튼을 제공합니다.
