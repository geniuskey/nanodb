...
- 라벨과 메모는 저장 후에도 함께 수정할 수 있다(ANN-003).
- 좌표는 '보정'으로 옮길 수 있다(2026-09-09 개정). 자동 추출이 100% 정확하지 않고 손으로 찍은
  점도 빗나갈 수 있기 때문이다. 보정은 침묵하지 않는다: 값은 서버가 옮긴 점으로부터 다시
  계산하고, 그 측정이 처음 가졌던 좌표·값은 보존되어 나란히 보이며, 되돌릴 수 있다. 종류와
  보정값(nm/pixel)은 여전히 불변이다. RES-007은 '좌표 불변'에서 '좌표는 보정 가능하되 모든
  보정은 기록되고 되돌릴 수 있다'로 좁혀진다.
...
이미지 클릭 외에 시작점·끝점의 원본 좌표를 수치로 입력해 draft를 만들 수 있다(UIX-003).
포인터 없는 유일한 측정 경로이자, 이미지가 원본보다 작게 표시될 때 정확한 픽셀을 지정하는
수단을 겸한다. 범위 밖이거나 두 점이 같으면 이유를 표시하고 draft를 만들지 않는다.

### 로딩 중 자리 확보

조회 중에는 결과가 들어올 자리를 같은 모양의 자리 표시로 채운다(UIX-009). 자리 표시는
장식이므로 `aria-hidden`이며, 상태 안내는 기존 `role="status"` 텍스트가 담당한다.
