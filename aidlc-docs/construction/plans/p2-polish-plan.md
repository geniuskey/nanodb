...
## 후속 완료 (2026-09-08, 같은 날)

보류했던 두 항목도 마무리해 P2 그룹을 전부 닫았다.

- **UIX-003**: 측정 화면에 `좌표로 직접 지정`을 추가했다. 원본 좌표 네 개를 입력해 draft를
  만들며, 범위 밖이거나 두 점이 같으면 이유를 표시하고 draft를 만들지 않는다. 포인터 없는
  측정 경로이자, 이미지가 축소돼 화면에서 집을 수 없는 픽셀을 지정하는 수단을 겸한다.
- **UIX-009**: 목록과 홈 KPI의 첫 조회에 같은 모양의 자리 표시를 넣어 결과 도착 시 화면이
  밀리지 않게 했다. 자리 표시는 `aria-hidden`이고 안내는 기존 `role="status"` 텍스트가 맡는다.

검증: ruff, mypy, pytest 104(로컬 PostgreSQL), vitest 66, build, e2e 7. 좌표 입력은 실제
브라우저에서 400×300 이미지에 (40,40)~(300,200)을 입력해 305.29px · 152.64nm draft가
생성되는 것까지 확인했다.
