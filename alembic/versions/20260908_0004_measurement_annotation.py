"""Move annotation onto the measurement it describes.

The standalone ``annotations`` table stored arrow and circle shapes that
carried no measured value, plus their own ``product`` and ``step`` columns
that contradicted the image the shapes were drawn on. It is replaced by two
columns on the rows that own the information: ``measurements.label`` names
what a measurement is, and ``images.process_step`` records the process step
of the whole image.
"""
...
def downgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("kind IN ('arrow', 'circle')", name="ck_annotations_kind"),
        sa.CheckConstraint(
    ...
    op.drop_column("images", "process_step")
