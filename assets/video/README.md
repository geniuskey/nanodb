...
| 파일 | 코덱 | 쓰는 곳 |
| --- | --- | --- |
| `nanodb_intro.mp4` | HEVC(H.265, `hvc1`) · 1920x1080 · 35s · 무음 | 앱 홈(`src/frontend/src/pages/HomePage.tsx`)이 번들에 포함해 재생 |
| `nanodb_intro_h264.mp4` | H.264(High, `avc1`) · 같은 영상 · `+faststart` | GitHub Pages 근거 사이트 홈(`docs/index.md`) |
| `nanodb_intro_poster.jpg` | 영상 4초 지점 프레임 · 1280px | 같은 홈의 `poster`(영상 로드 전·재생 불가 시 표시) |

## 왜 두 벌인가

원본은 HEVC입니다. HEVC는 Safari에서는 재생되지만 **Chrome·Edge는 플랫폼에 하드웨어
HEVC 디코더가 있을 때만**, Firefox는 대부분 재생하지 못합니다. 공개된 Pages 사이트는
방문자의 브라우저를 고를 수 없으므로, 어디서나 재생되는 H.264 사본을 사이트용으로 둡니다.
