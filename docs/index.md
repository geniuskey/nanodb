...
> [!TIP] 후속 개발자라면
> 이 저장소를 이어받아 개발을 계속하려는 분은 **[개발자 가이드](/guide/)** 부터 보세요.
> 아키텍처·백엔드·프론트엔드·데이터 모델·API·컨텍스트 내보내기·테스트/배포·확장 방법을
> 코드에 근거해 정리했습니다. 사람이 떠나더라도 개발이 이어지도록 하는 것이 이 문서의 목표입니다.
...
- 이미지(PNG·JPEG·TIFF)를 제조 메타데이터·`nm/pixel` 보정값과 함께 등록
- 등록 이미지를 목록에서 최신순·키워드·종류로 찾아 다시 열기
- 세그멘테이션(multi-Otsu)과 자동 특징 추출로 폭·높이·간격·반경·측벽각 등을 길이·각도·곡률
  측정값으로 자동 생성 — 사람이 검증하지 않은 **미검토 참고값**으로 구분 저장(신뢰도 표기)
...
2. **Measurement** — 세그멘테이션과 자동 특징 추출을 실행하면 길이·각도·곡률 측정값이
   자동으로 만들어집니다(미검토 참고값). 필요하면 기준점을 직접 보정해 값을 다시 계산하며,
   저장된 측정은 원본 좌표로 복원됩니다.
...
- 인증·권한은 이번 범위 밖입니다. 단일 키워드 검색·종류 필터, 측정·이미지 삭제, 항목별 표본
  수·평균·최소·최대, 측정 라벨링, 확대·이동, 라벨·메모 수정은 P1으로 구현을 마쳤습니다.
- 저장된 측정은 라벨과 메모만 수정할 수 있습니다. 좌표·항목·값·보정값은 측정 근거이므로 불변입니다.
- 태블릿·모바일 터치 측정과 다크모드는 지원 범위가 아닙니다.
- 측정값은 자동 세그멘테이션·특징 추출로 생성되며, 사람이 검증하지 않은 **미검토 참고값**입니다.
  자동 계측의 정답(ground truth)으로 취급하지 않습니다. 수동 두 점 측정 UI는 코드에 남아 있으나
  현재 비활성(`MANUAL_MEASUREMENT_ENABLED = false`)이고, 저장된 자동 측정의 기준점 보정만
  제공합니다.
