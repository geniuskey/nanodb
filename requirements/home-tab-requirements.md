...
- **HOM-002:** 데스크톱 헤더에는 슬로건 `사람은 떠나도 데이터는 남는다`를 표시해야 한다.
...
- **HOM-007:** 본문은 다음 순서를 유지해야 한다.
  ...
  7. 사용 흐름
  8. 데이터 정책
- **HOM-008:** 대표 제목은 `만드는 사람은 바뀌어도, 데이터와 도구는 회사에 남게`의 뜻을 유지해야 한다.
...
- **HOM-016:** `왜 지금인가` 영역은 최대 네 개의 짧은 카드로 제한한다.
- **HOM-017:** 각 카드에는 수치, 수치가 뜻하는 바, 출처 기관, 발표 연도를 표시해야 한다.
- **HOM-018:** 외부 통계는 정적 콘텐츠로 제공하며 런타임 외부 API에 의존하지 않아야 한다.
- **HOM-019:** 오래된 조사 수치를 현재 실측값처럼 표현하지 않아야 하며, 조사 연도를 숨기지 않아야 한다.
- **HOM-020:** 통계 상세 URL은 새 탭 링크 또는 출처 표기로 제공할 수 있다. 공간이 부족하면 네 개 카드 중 일부를 제거할 수 있다.

### 3.5 실제 KPI

- **HOM-021:** 실제 KPI 영역은 최소 전체 이미지 수와 전체 측정 수를 서버 집계 결과로 표시해야 한다.
- **HOM-022:** 항목별 측정 카드에는 측정 종류, 평균값, 단위 `nm`, 표본 수 `n`을 표시해야 한다.
