...
### 7.7 2026-09-07 개정 2 — 심사 흐름 구성, 링크 제거, 이미지 사용

- **홈은 다른 탭으로 가는 링크·버튼을 두지 않는다.** 이동은 주 내비게이션만 담당한다. 히어로 CTA 줄, "한 줄로 물어보기" 위젯, 패널의 Analysis/Dashboard 버튼, 차트 드릴다운을 모두 제거했다 (HOM-012~015는 내비 항목으로 충족).
- **본문 순서(개정)**: 대의와 세 가치 → 지금 쌓인 데이터(실데이터 KPI, 구성 막대, 등록 추이) → 최근 등록된 이미지(썸네일 6장) → 분포 히스토그램과 Lot별 박스플롯 → 왜 지금인가(아이콘 + 수치 + 출처·연도) → Phase 1~4(삽화) → 어떻게 만들었나(AI-DLC 심사 포인트 3개: 산출물 구조, 핵심 설계 결정, 협업하며 배운 것) → 이렇게 쓰세요 → 정책 줄.
- **문장 부호**: 가운뎃점(·)을 쓰지 않는다. 쉼표와 마침표로 쓴다.
- **이미지**: 헤더 로고는 태그라인이 없는 `art/nanodb_logo_mark.png`(22px). 히어로는 태그라인 포함 로고(88px). Phase 카드는 `art/phase1_collect.svg` 등 4장, 근거 카드는 `art/ev_people.svg` 등 4장. 최근 이미지는 카탈로그 썸네일 API를 그대로 쓴다.
- 실행 화면: ![홈 실행 화면(개정 2)](home-tab/home-live.png), 참고 구현 [`home-tab/home.reference.js`](home-tab/home.reference.js)
